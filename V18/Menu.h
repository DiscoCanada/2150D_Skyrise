...
int sensor_menu()
{
	...
	while (!picked2)
	{
		liftHeight = "";
		limitswitch = "";
		if (SensorValue[rtZero] == 1)
		{
			nMotorEncoder[aBotR] = 0;
		}
		displayLCDCenteredString(1, "<Moar | Back>");
		displayLCDString(0, 0, "lift: ");
		sprintf(liftHeight, "%d", nMotorEncoder[aBotR]); //Build the value to be displayed
		displayLCDString(0, 4, liftHeight);
		sprintf(limitswitch, "%d", SensorValue[rtZero]); //Build the value to be displayed
