...
const tMotor kDriveMotorPort[2][kDriveMotorsPerSide] = {{port1, port2}, {port3, port4}}; // {{left motors}, {right motors}}
...
task drivePID() {
  ...
  while(true) {
    ...
    for(ubyte j=0; j<kDriveMotorsPerSide; j++) {
      motor[kDriveMotorPort[0][j]] = speedL;
      motor[kDriveMotorPort[1][j]] = speedR;
    }
    prevError = error;
